...
import { countryDataType, multiStepFormDataType, personalInfoType } from "@/types/FormTypes"
...
const PersonalInfoForm = ({ data, saveData, setContinue }: { data: personalInfoType, saveData: React.Dispatch<React.SetStateAction<multiStepFormDataType>>,
    ...
    function handleChange(e: ChangeEvent<HTMLInputElement> | countryDataType){
        ...
        if(errorMessages.errorEmail || errorMessages.errorName || errorMessages.errorPhone )
            setErrorMessages({})
        ...
        if(!('phoneCode' in e)){
            ...
            if(e.target.name === 'phone'){
                ...
                userInfo[e.target.name] = value
        }
        else
            userInfo.phone = { ...phone, countryData: e }
    ...
    function checkValidValues(e: FormEvent<HTMLFormElement>){
        ...
        if(!name)
            return setErrorMessages({errorName: 'This field is required' })
        if(!email || !(/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/).exec(email))      
            return setErrorMessages({errorEmail: !email ? 'This field is required' : 'Invalid Email' })

        if(!phone.number || !phone.countryData.name || !phone.countryData.phoneCode)
            return setErrorMessages({errorPhone: !phone.countryData.phoneCode ? 'Select a Country code' : 'This field is required' })
